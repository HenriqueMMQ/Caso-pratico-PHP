        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (i = 0; i < objeto_json.items.length; i++) {
                    frase = frase + "<br/><b>" + objeto_json.items[i].title + "</b><br/>"; frase = frase + objeto_json.items[i].description + "<br/>";
                }
    ...
    if (open_zdnet == false) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                phrase = '';
                for (i = 0; i < objeto_json.items.length; i++) {
                    phrase = phrase + "<b>" + objeto_json.items[i].title + "</b><br/>"; phrase = phrase + objeto_json.items[i].description + "<br/>";
                    phrase = phrase + '<a href="' + objeto_json.items[i].link + '">' + objeto_json.items[i].link + '<a/><br/><br/>'
...
function check_checkbox() {
    checked_checkboxes = document.querySelectorAll('input[type="checkbox"]:checked').length
    $("#final_price").val("€ " + ((checked_checkboxes * 400) - ((checked_checkboxes * 400) * discount)))
}
...
/* function validate(form) {
    ...
    if (valid_reason == true && valid_email == true && valid_phone == true && valid_nick == true && valid_name == true) {
        form.submit();
    } else {
...
function validate_login(form) {
    ...
    if (valid_email == true && valid_password == true) {
        ...
        //mensagem de erro a dizer que os dados inseridos estão incorretos
        return false;
    }
...
function validate_register(form) {
    ...
    if (valid_name == true && valid_nick == true && valid_email == true && valid_password == true) {
        form.submit();
    } else {
        alert("O formulário contém erros.")
        return false;
